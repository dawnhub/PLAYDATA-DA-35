...
select * from emp where job_id = (select job_id from emp where emp_id = 115) and dept_id = (select dept_id from emp where emp_id = 115);
...
select d.dept_name, concat('$', format(avg(e.salary),2))
...
select *
from emp
where salary > (select avg(e.salary) from emp e join dept d on e.dept_id = d.dept_id where d.dept_name = 'Sales');
...
and (job_id != 'st_clerk' or job_id is null); -- 주의, st_clerk가 아닌지 물을때 null과 연산하면 null이라 포함안함
